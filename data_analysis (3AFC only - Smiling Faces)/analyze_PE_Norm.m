function analyze_PE_Norm(chosenFaces_PE_Norm_All, subjectOrPatient, saveFigure)
    ...
    disp('t-test for highPE_highNorm_percentChosenTarget vs highPE_lowNorm_percentChosenTarget:');
    ...
    % t-test for HPE: lowNorm vs highNorm
    disp('t-test for lowPE_highNorm_percentChosenTarget vs lowPE_lowNorm_percentChosenTarget:');
    ...
    if(saveFigure)
        ...
        savingFileName = ['overall_PEVsNorm_%ChosenTarget_' subjectOrPatient 's_(n=' num2str(length(highPE_highNorm_percentChosenTarget)) ').jpg'];
